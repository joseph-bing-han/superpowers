...
assert_section_uses_canonical_carrier_model() {
  ...
  assert_section_contains "$file" "$heading" 'visible tail block.*fallback|tail block.*only a fallback|用户可见.*tail block.*fallback|用户可见.*tail block.*回退' "$description_prefix limits visible tail blocks to fallback-only status" "$stop_pattern"
  assert_section_not_contains "$file" "$heading" 'last 4 non-empty lines before the next machine action must be (the|that) canonical `ENDGATE_\*` packet' "$description_prefix no longer treats a user-visible tail block as the only canonical path" "$stop_pattern"
}
