...
assert_section_contains "spec-governed-development/SKILL.md" "## Handoff Guidance" '1\. 结束 \(Recommended\).*2\. 继续' "spec-governed-development requires the terminal-choice popup to author only stop and continue" '^## '
assert_section_contains "spec-governed-development/SKILL.md" "## Handoff Guidance" 'client-provided `Other` / notes path|客户端自动追加的 `Other` / notes 路径' "spec-governed-development routes terminal free-form input through the client-provided Other/notes path" '^## '
assert_section_not_contains "spec-governed-development/SKILL.md" "## Handoff Guidance" '3\. 自由输入|显式.*自由输入|author(?:ed|ing).*自由输入' "spec-governed-development does not preserve a duplicate authored terminal free-form option" '^## '
assert_section_contains "spec-governed-development/SKILL.md" "## Handoff Guidance" '当 handoff 进入 `terminal-choice` 时.*直接下一个动作.*`request_user_input`|直接下一个动作.*`request_user_input`.*terminal-choice' "spec-governed-development makes terminal-choice popup-first instead of prose-first" '^## '
assert_section_contains "spec-governed-development/SKILL.md" "## Handoff Guidance" '不要在 terminal-choice 弹窗前先输出.*当前我建议的定稿.*就按这条落地.*最终建议一句话版|terminal-choice 弹窗前.*当前我建议的定稿.*就按这条落地.*最终建议一句话版' "spec-governed-development forbids terminal prose closeouts before the popup" '^## '
assert_section_contains "spec-governed-development/SKILL.md" "## Handoff Guidance" '稳定建议、最终定稿或最终摘要.*不构成直接结束许可|不构成直接结束许可.*稳定建议.*最终定稿.*最终摘要' "spec-governed-development treats settled recommendations and final summaries as non-terminal until the popup runs" '^## '
