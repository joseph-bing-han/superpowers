...
assert_section_contains "skills/using-superpowers/SKILL.md" "## Autonomous Continuation" 'if the user asked for end-to-end completion.*no clarification.*no confirmation.*continue automatically|continue automatically.*if the user asked for end-to-end completion.*no clarification.*no confirmation' "global contract auto-continues end-to-end work when no clarification or confirmation is needed" '^## '
...
assert_section_contains "skills/subagent-driven-development/SKILL.md" "## The Process" 'after a task passes review.*move directly to the next task|task summaries are progress updates, not requests for permission to continue|do not stop after a reviewed task summary if more tasks remain' "subagent-driven-development continues after reviewed task summaries" '^## '
assert_section_contains "docs/README.codex.md" "## Autonomous Continuation" 'end-to-end completion.*continue automatically when no clarification or confirmation is needed|continue automatically.*end-to-end completion.*no clarification.*no confirmation' "Codex README documents the autonomous continuation contract" '^## '
