...
assert_section_contains "skills/using-superpowers/SKILL.md" "## User Choice Formatting" 'When you write prose-numbered options or text fallbacks yourself, use ASCII `1\. `, `2\. `, and `3\. ` numbering' "global rule locks prose-numbered options to ASCII numbering" '^## '
...
assert_section_contains "docs/README.codex.md" "## Choice-Based Interaction" 'When the assistant writes prose-numbered options or text fallbacks itself, use ASCII `1\. `, `2\. `, and `3\. ` numbering instead of `1。`' "Codex docs require ASCII numbering for assistant-authored prose choices"
...
assert_section_contains "docs/README.codex.md" "## Choice-Based Interaction" 'The automatic numbering prefix shown inside a `request_user_input` popup belongs to the Codex UI/input-layer boundary, not to the skill documents' "Codex docs document the request_user_input automatic-numbering boundary"
assert_section_not_contains "docs/README.codex.md" "## Choice-Based Interaction" 'automatic numbering prefix.*(can|is) controlled by.*skill documents|skill documents.*control.*automatic numbering prefix|request_user_input.*popup.*prefix.*configurable.*skill' "Codex docs do not claim that skill documents control the request_user_input popup prefix"
...
assert_section_contains "docs/testing.md" "### Required Evidence" 'assistant-authored payload 只包含 `结束 \(Recommended\)`、`继续`；客户端 UI 会自动追加 `Other` / notes path 作为自由输入兜底，不应把它记录成 assistant-authored `3`。' "testing docs separate terminal-choice authored payload from the client UI fallback" '^(##|###) '
