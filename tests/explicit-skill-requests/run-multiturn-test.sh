...
# Turn 3: The critical test - ask for systematic-debugging
echo ">>> Turn 3: Requesting systematic-debugging..."
TURN3_LOG="$OUTPUT_DIR/turn3.json"
claude -p "use systematic-debugging" \
    --continue \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
    --max-turns 2 \
...
SKILL_PATTERN='"skill":"([^"]*:)?systematic-debugging"'
if grep -q '"name":"Skill"' "$TURN3_LOG" && grep -qE "$SKILL_PATTERN" "$TURN3_LOG"; then
    echo "PASS: Skill 'systematic-debugging' was triggered in Turn 3"
    TRIGGERED=true
else
    echo "FAIL: Skill 'systematic-debugging' was NOT triggered in Turn 3"
    TRIGGERED=false
fi
