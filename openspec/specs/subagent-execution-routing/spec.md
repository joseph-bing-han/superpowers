...
`Pipeline SDD` 与 `Parallel Dispatch` 三种模式，而不是把“有子代理”默认为
最大并发。
...
#### Scenario: Default same-session execution selects Pipeline SDD
- **WHEN** 任务位于 same-session 子代理执行路径
- **AND** 没有满足 `Parallel Dispatch` 的安全条件
- **THEN** 默认模式 MUST 是 `Pipeline SDD`

#### Scenario: Disjoint lanes can upgrade to Parallel Dispatch
- **WHEN** 计划证明多个 lane 具备独立的 `Write Set`
  与 `Conflict Group` 边界
- **THEN** 路由 MAY 升级到 `Parallel Dispatch`
