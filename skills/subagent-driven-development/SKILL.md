...
- Every reviewed-task boundary in this repository uses `endgate-state-packet`; treat the last packet plus its post-packet event window as the governing runtime contract for that boundary.
...
- At every reviewed-task boundary in this repository, emit this exact packet immediately before the next machine action:
```text
...
- `AUTO_CONTINUE`: emit the packet, then immediately move into the next reviewed task.
- `NEEDS_USER_DECISION`: emit the packet, then immediately call `request_user_input` with the concrete next-task or blocker-resolution options.
