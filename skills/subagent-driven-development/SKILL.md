...
- Every reviewed-task boundary in this repository is carrier-backed; treat the last canonical carrier plus its post-carrier event window as the governing runtime contract for that boundary.
...
- At every reviewed-task boundary in this repository, ensure the canonical machine-readable carrier contains this exact packet before the next machine action:
```text
...
- `AUTO_CONTINUE`: ensure the canonical carrier records this packet, then immediately move into the next reviewed task.
- `NEEDS_USER_DECISION`: ensure the canonical carrier records this packet, then immediately call `request_user_input` with the concrete next-task or blocker-resolution options.
